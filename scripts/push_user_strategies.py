...
用法：
  ...
  # 4. 雲端模式（從 admin endpoint 撈，不需要 wrangler）
  PUSH_API_BASE=https://feature-favorites-v2.taiwan-stock-scanner.pages.dev \
  INTERNAL_CRON_TOKEN=xxx \
  python scripts/push_user_strategies.py --source endpoint
...
旗標：
  ...
  --source d1|endpoint  資料來源（預設 d1，本地走 wrangler；endpoint 走 HTTPS）

需要 .env 設定：
  TELEGRAM_BOT_TOKEN
  PUSH_API_BASE         （endpoint mode 必需，例如 https://...pages.dev）
  INTERNAL_CRON_TOKEN   （endpoint mode 必需）
...
def load_env() -> Dict[str, str]:
    ...
    # os.environ 優先（CI/cron 用），讓 GitHub Secrets 蓋過 .env
    for k in ("TELEGRAM_BOT_TOKEN", "INTERNAL_CRON_TOKEN", "PUSH_API_BASE",
              "TELEGRAM_CHAT_ID"):
...
# ============================================================
#  資料來源 1：從 admin endpoint 撈（雲端 cron 用，也支援本地）
# ============================================================
def fetch_endpoint_data(api_base: str, token: str) -> List[Dict[str, Any]]:
    """打 GET /api/internal/cron/bound-users，回 users array。"""
    url = api_base.rstrip("/") + "/api/internal/cron/bound-users"
    logger.info("撈 endpoint：%s", url)
    try:
        r = requests.get(
            url,
        ...
        logger.error("endpoint request 例外：%s", e)
        sys.exit(2)
    if not r.ok:
        logger.error("endpoint 回 %d：%s", r.status_code, r.text[:300])
        sys.exit(2)
    try:
        payload = r.json()
    except Exception as e:
        logger.error("endpoint 回應不是 JSON：%s\n%s", e, r.text[:300])
        sys.exit(2)
...
# ============================================================
#  資料來源 2：從 D1 撈（本地 wrangler，不需網路 endpoint）
# ============================================================
...
def run() -> int:
    ...
    if args.source == "endpoint":
        ...
        if not api_base or not token:
            logger.error("--source endpoint 需要 PUSH_API_BASE + INTERNAL_CRON_TOKEN env")
            return 1
        users = fetch_endpoint_data(api_base, token)
        logger.info("endpoint 撈到 %d 位綁定使用者", len(users))
    else:
        users = fetch_d1_data()
